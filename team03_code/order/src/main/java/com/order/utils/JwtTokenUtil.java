...
public class JwtTokenUtil {
	 ...
	 * @return
	 */
	public String createJwt(Integer uid, String uname) {
		Date now = clock.now();
		...
		JwtBuilder jwtBuilder = Jwts.builder().setHeaderParam("typ", "JWT").setIssuedAt(now) // 设置jwt创建时间
				.setIssuer(uname) // 设置当前用户信息
				.setExpiration(calculateExpirationDate(now)) // 设置失效时间
				.setSubject("uname")
				.claim("uid",uid)//通过claim()方法可添加若干属性，也可以添加一个对象
				.claim("uname",uname)//通过claim()方法可添加若干属性，也可以添加一个对象
