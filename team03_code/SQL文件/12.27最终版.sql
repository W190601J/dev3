...
CREATE TABLE `chef` (
  `chid` int(11) NOT NULL AUTO_INCREMENT COMMENT '厨师ID',
...
CREATE TABLE `class` (
  `cid` int(11) NOT NULL AUTO_INCREMENT COMMENT '菜品类ID',
  ...
  PRIMARY KEY (`cid`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='菜品分类表';
...
CREATE TABLE `food` (
  `fid` int(32) NOT NULL AUTO_INCREMENT COMMENT '菜品ID',
...
CREATE TABLE `ordering` (
  `oid` int(32) NOT NULL AUTO_INCREMENT COMMENT '订单ID',
  ...
  `updatetime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '订单修改时间',
  `beiyong1` varchar(64) COLLATE utf8_bin DEFAULT NULL COMMENT '备用1',
  PRIMARY KEY (`oid`)
...
CREATE TABLE `users` (
  `uid` int(32) NOT NULL AUTO_INCREMENT COMMENT '用户ID',
  ...
  `userupdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '用户更新时间',
  `rank` int(11) NOT NULL COMMENT '用户级别  0：管理员 1：普通用户 2:老板',
  PRIMARY KEY (`uid`)
